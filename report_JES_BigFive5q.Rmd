...
* Valor de los resultados aproximados VS “pesadez” de 50 preguntas = interés en lograr resultados aprox con sólo 5 preguntas -> defino una función de Accuracy HighLow por trait acertando >=3 de 5 preguntas
...
Correlation within same group: Questions explitly associated to each "trait" strongly correlate, i.e. all of them are related because all of them explain the final value of the score for its specific group (trait). We see now two examples two different traits (groups). Correlation is clearly high within same group.
...
* INICIAL: comprobar si “reverse questions” afectan al recommender (estudiar impacto en: 

1) recommenderPrincipal
2)recommenderShiny
3) cálculoOCEANinicial
4) cálculoOCEANshiny || fórmula: abs(6-x)
...
![Screenshot1 of interactive tool](shinnyapp_image1.jpg) ![Screenshot2 of interactive tool](shinnyapp_image2.jpg)
