...
We see how accuracies, as defined, get significantly improved with both ALS and UBCF methods algorithms respect to base "random" reference generated with the Montecarlo approach. As a final high level number of prediction improvement, the improvement in global accuracy reach an impressing 57% when consolidation all measured accuracies.
...
Project has combined strong **academic premises** (clear separation train/validation, exact definition and measure of accuracy, realistic computability) with some **very original aspects**, like the fact of using a *Recommender* model as base, the algorithm to combine random and optimal questions to retrieve answer from or the interactive web app implementation.
